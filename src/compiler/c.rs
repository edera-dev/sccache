...
fn find_asm_dependencies(bytes: &[u8]) -> Vec<AsmDependency> {
    ...
    while offset < bytes.len() {
        let Some(pos) = [INCBIN_DIRECTIVE, INCLUDE_DIRECTIVE]
            .iter()
            .filter_map(|directive| memchr::memmem::find(&bytes[offset..], directive))
            .min()
        else {
            break;
        };
...
fn process_preprocessed_file(
    ...
    while start < total_len.saturating_sub(7) {
        ...
        } else if asm_dependency_at(slice).is_some() {
